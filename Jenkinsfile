...
def buildAndPushMyDockerImage(config) {
    ...
    if (config.gitTagName) {
    ...
    } else if (config.branchName == 'main') {
        ...
        echo "Building from 'main' branch. Image tag will be: '${buildTag}'"
    ...
    docker.withRegistry('https://index.docker.io/v1/', config.dockerHubCredsId) {
        ...
        image.push(buildTag)
        echo "⬆️Pushed ${config.imageName}:${buildTag} to Docker Hub."
        ...
        if (shouldPushLatest) {
            ...
            image.push("latest")
            echo "⬆️Pushed ${config.imageName}:latest to Docker Hub (from main branch)."
        }
        ...
        stage('Build and Push Docker Image') {
            steps {
                script {
                    ...
                    def gitTagName = env.GIT_TAG_NAME // Sarà null se non è una build da tag
                    def branchName = env.BRANCH_NAME
                    def gitCommitShortSha = env.GIT_COMMIT.substring(0, 7) // I primi 7 caratteri dell'SHA
